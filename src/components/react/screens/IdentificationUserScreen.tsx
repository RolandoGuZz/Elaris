import { InputText } from "../form/InputText";
import { InputSelect } from "../form/InputSelect";
import { SelectSex } from "../form/SelectSex";
import { SelectState } from "../form/SelectState";
import { MapComponent } from "../layout/MapComponent";
import { getBirthDateBounds } from "../core/validations/utils/birthDate";
import { normalizeCurp } from "../core/validations/utils/curp";

const CAREER_OPTIONS = [
  "Ingeniería de Software",
  "Administración",
  "Agronomía",
];
const CAMPUS_OPTIONS = ["Nopala", "Juxtlahuaca", "San Jacinto"];

export const IdentificationUserScreen = () => {
  const { max, min } = getBirthDateBounds();

  return (
    <>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <InputText
          label="Nombre"
          name="identification.firstName"
          placeholder="Ingresa tu nombre"
        />

        <InputText
          label="Apellidos"
          name="identification.lastName"
          placeholder="Ingresa tus apellidos"
        />
        <InputText
          label="Fecha de Nacimiento"
          name="identification.birthDate"
          type="date"
          max={max}
          min={min}
        />
        <div className="flex flex-col gap-2">
          <div className="flex items-center justify-between">
            <span className="text-sm font-semibold text-left text-slate-700 dark:text-slate-300">
              CURP
            </span>
            <a
              href="https://www.gob.mx/curp/"
              target="_blank"
              rel="noopener noreferrer"
              className="text-xs font-medium text-primary text-sky-500 underline"
            >
              ¿No conoces tu CURP? Puedes consultarla aquí.
            </a>
          </div>
          <InputText
            name="identification.curp"
            type="text"
            placeholder="Ingresa tu CURP"
            maxLength={18}
            transformUppercase
            rules={{
              setValueAs: (value: string) => normalizeCurp(value ?? ""),
            }}
          />
        </div>
      </div>

      <div className="pt-4 grid grid-cols-1 md:grid-cols-2 gap-6 mb-3">
        <InputText
          label="Correo electronico"
          name="identification.email"
          type="email"
          placeholder="ejemplo@email.com"
        />
        <InputText
          label="Telefono"
          name="identification.phone"
          type="number"
          placeholder="e.g. 5551234567"
        />

        <InputSelect
          label="Selecciona la carrera a la que quieres entrar"
          name="identification.career"
          options={CAREER_OPTIONS}
        />
        <InputSelect
          label="Campus"
          name="identification.campus"
          options={CAMPUS_OPTIONS}
        />
        <SelectSex label="Sexo" name="identification.gender" />
        <SelectState label="Estado Civil" name="identification.maritalStatus" />
      </div>
      <MapComponent
        title="Indica tu domicilio en el mapa"
        name="identification.address"
      />
    </>
  );
};
